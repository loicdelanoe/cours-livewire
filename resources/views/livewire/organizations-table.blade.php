<main class="main-content px-4 py-8 md:flex-1 md:p-12 md:overflow-y-auto">
    <h1 class="mb-8 text-3xl font-bold">Organizations</h1>
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center mr-4 w-full max-w-md">
            <form class="flex w-full bg-white rounded shadow" x-data="{ searchButton: false }">
                <button type="button"
                        class="focus:z-10 px-4 hover:bg-gray-100 border-r focus:border-white rounded-l focus:ring md:px-6">
                    <div class="flex items-baseline"><span class="hidden text-gray-700 md:inline">Filter</span>
                        <svg class="w-2 h-2 fill-gray-700 md:ml-2"
                             xmlns="http://www.w3.org/2000/svg"
                             viewBox="0 0 961.243 599.998">
                            <path
                                d="M239.998 239.999L0 0h961.243L721.246 240c-131.999 132-240.28 240-240.624 239.999-.345-.001-108.625-108.001-240.624-240z"></path>
                        </svg>
                    </div>
                </button>
                <input class="relative px-6 py-3 w-full rounded-r focus:shadow-outline"
                       autocomplete="off"
                       type="text"
                       name="search"
                       placeholder="Name"
                       wire:model.live.="search"
                >
                <button x-show="searchButton" type="submit"
                        class="focus:z-10 px-4 hover:bg-gray-100 border-r focus:border-white rounded-l focus:ring md:px-6"
                        disabled>
                    Search
                </button>
            </form>
        </div>
        <a class="btn-indigo"
           href="/organizations/create" wire:navigate.hover><span>Create</span><span class="hidden md:inline">&nbsp;Organization</span></a>
    </div>
    <div class="bg-white rounded-md shadow overflow-x-auto">
        <table class="w-full whitespace-nowrap">
            <thead>
            <tr class="text-left font-bold">
                <th class="pb-4 pt-6 px-6">Name</th>
                <th class="pb-4 pt-6 px-6">City</th>
                <th class="pb-4 pt-6 px-6"
                    colspan="2">Phone
                </th>
            </tr>
            </thead>
            <tbody>
            @foreach($this->organizations as $organization)
                <tr class="hover:bg-gray-100 focus-within:bg-gray-100 relative">
                    <td class="border-t">
                        <a class="absolute inset-0"
                           href="/organizations/{{ $organization->id }}/edit" wire:navigate
                           wire:key="{{ $organization->id }}">
                            <span class="sr-only px-6 py-4 focus:text-indigo-500">{{ $organization->name }}</span>
                        </a>
                        <span class="inline-block px-6 py-4">{{ $organization->name }}</span>
                    </td>
                    <td class="border-t">
                        <span class="inline-block px-6 py-4">{{ $organization->city }}</span>
                    </td>
                    <td class="border-t">
                        <span class="inline-block px-6 py-4">{{ $organization->phone }}</span>
                    </td>
                    <td class="w-px border-t px-4">
                        <svg xmlns="http://www.w3.org/2000/svg"
                             viewBox="0 0 20 20"
                             class="block w-6 h-6 fill-gray-400">
                            <polygon
                                points="12.95 10.707 13.657 10 8 4.343 6.586 5.757 10.828 10 6.586 14.243 8 15.657 12.95 10.707"></polygon>
                        </svg>
                    </td>
                </tr>
            @endforeach
            </tbody>
        </table>
    </div>
    {{ $this->organizations->links('vendor.livewire.ping', ['scrollTo' => false]) }}
</main>
